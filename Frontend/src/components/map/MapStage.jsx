  ...
  return (
    <div className="relative flex-1 h-[calc(100vh-3.5rem)] bg-[#040813] overflow-hidden select-none">
      ...
      >
        ...
        {/* Real Destinations Layer from MongoDB */}
        {layers.destinations && allDestinations && allDestinations.length > 0 && (
          allDestinations.map((dest) => (
            <DestinationMarker 
              key={`dest-${dest._id || dest.slug}`}
              destination={dest}
            />
          ))
        )}

        {/* Real Spiritual Sites Layer from MongoDB */}
        {layers.spiritual && allSpiritual && allSpiritual.length > 0 && (
          allSpiritual.map((site) => (
            <DestinationMarker 
              key={`spir-${site._id || site.slug}`}
              destination={site}
            />
          ))
        )}

        {/* Real Adventure Activities Layer from MongoDB */}
        {layers.activities && allActivities && allActivities.length > 0 && (
          allActivities.map((act) => (
            <DestinationMarker 
              key={`act-${act._id || act.slug}`}
              destination={act}
            />
          ))
        )}
