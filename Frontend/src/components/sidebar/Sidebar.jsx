      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4 custom-scrollbar">
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5">
                  {filteredDestinations.map((dest) => {
                    ...
                    const isSelected = (selectedDestination?._id || selectedDestination?.id) === destId;
                    return (
                      <div
                        key={destId}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] text-slate-400 truncate m-0">
                            {dest.district || 'Uttarakhand'} {dest.altitude ? `• ${dest.altitude}m` : ''}
                          </p>
                          {dest.bestSeason && (
          ...
          {activeSidebarTab === 'spiritual' && (
            <div className="space-y-4">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 block mb-1">
                Sacred Pilgrimages & Confluences ({allSpiritual?.length || 52})
              </span>

              {allSpiritual && allSpiritual.length > 0 ? (
                <div className="space-y-2">
                  {allSpiritual.map((site) => (
                    <div
                      ...
                      className="p-2.5 rounded-xl bg-slate-900/50 hover:bg-slate-800/80 border border-slate-800/80 cursor-pointer flex items-center gap-3 transition-all"
                    >
                      <img
                      ...
                      />
                      <div className="flex-1 min-w-0">
                        <h5 className="text-xs font-bold text-amber-300 m-0 truncate leading-snug">
                          {site.name}
                        </h5>
                        <p className="text-[10px] text-slate-400 m-0 mt-0.5 truncate">
                          {site.district || 'Uttarakhand'} {site.elevation ? `• ${site.elevation}m` : ''}
                        </p>
                      </div>
                    </div>
                  ))}
