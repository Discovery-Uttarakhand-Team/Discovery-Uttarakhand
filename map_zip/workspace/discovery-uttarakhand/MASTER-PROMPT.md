...
Expected totals: **37 + 26 + 7 + 20 + 51 + 9 + 190 = 340**. Slugs are globally unique across all 7 files.
...
Cloudinary upload is pending manifest approval. When approved, a migration script will: read `backend/seed/image-manifest.json` (668 images with `imageUrl`, `license`, `attribution`), upload each to Cloudinary under `discovery-uttarakhand/<entityType>/<entityId>`, then update every document's `Image.url` → Cloudinary URL and `Image.publicId` → the new ID, while **keeping `license`/`attribution` intact** (attribution obligations survive re-hosting). Design image fields now so this is a pure data migration.

---

## 7. ACCEPTANCE CHECKLIST

- [ ] `npm run seed` → 7 collections with exact counts 37/26/7/20/51/9/190; re-run creates no duplicates
